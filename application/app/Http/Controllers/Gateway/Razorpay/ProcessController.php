...
class ProcessController extends Controller
{
    ...
    public function ipn(Request $request)
    {
        ...
        $deposit = Deposit::where('btc_wallet', $request->input('razorpay_order_id'))->orderBy('id', 'DESC')->first();
        $razorAcc = json_decode($deposit->gatewayCurrency()->gateway_parameter);
        ...
        $sig = hash_hmac('sha256', $request->input('razorpay_order_id') . "|" . $request->input('razorpay_payment_id'), $razorAcc->key_secret);
        $deposit->detail = $request->all();
        $deposit->save();

        if ($sig == $request->input('razorpay_signature') && $deposit->status == '0') {
