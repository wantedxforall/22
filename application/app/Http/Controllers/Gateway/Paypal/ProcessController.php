...
class ProcessController extends Controller
{
    ...
    public function ipn(Request $request)
    {
        ...
        // $paypalURL = "https://ipnpb.sandbox.paypal.com/cgi-bin/webscr?"; // use for sandbox test
        $paypalURL = "https://ipnpb.paypal.com/cgi-bin/webscr?";
        $url = $paypalURL . $req;
        $response = CurlRequest::curlContent($url);

        if ($response == "VERIFIED") {
            $deposit = Deposit::where('trx', $request->input('custom'))->orderBy('id', 'DESC')->first();
            $deposit->detail = $details;
            $deposit->save();

            if ($request->input('mc_gross') == round($deposit->final_amo,2) && $deposit->status == '0') {
