...
class ProcessController extends Controller
{
    ...
    public function ipn(Request $request)
    {
        $request->validate([
            ...
            'TXNAMOUNT' => 'required',
            'RESPMSG' => 'nullable|string',
        ]);

        $deposit = Deposit::where('trx', $request->input('ORDERID'))->orderBy('id', 'DESC')->first();
        $PayTmAcc = json_decode($deposit->gatewayCurrency()->gateway_parameter);
        $ptm = new PayTM();

         if ($ptm->verifychecksum_e($request->all(), $PayTmAcc->merchant_key, $request->input('CHECKSUMHASH')) === "TRUE") {

             if ($request->input('RESPCODE') == "01") {
                $requestParamList = array("MID" => $PayTmAcc->MID, "ORDERID" => $request->input('ORDERID'));
                ...
                if ($responseParamList['STATUS'] == 'TXN_SUCCESS' && $responseParamList['TXNAMOUNT'] == $request->input('TXNAMOUNT')) {
                ...
                $notify[] = ['error',  $request->input('RESPMSG')];
            }
        } else {
