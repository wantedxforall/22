...
class ProcessController extends Controller
{
    ...
    public function ipn(Request $request)
    {
        ...
        $deposit = Deposit::where('btc_wallet', $request->input('payment_request_id'))->orderBy('id', 'DESC')->first();
        $instaMojoAcc = json_decode($deposit->gatewayCurrency()->gateway_parameter);
        $deposit->detail = $request->all();
        $deposit->save();
        $imData = $request->all();
        $macSent = $request->input('mac');
        unset($imData['mac']);
        ksort($imData, SORT_STRING | SORT_FLAG_CASE);
        $mac = hash_hmac("sha1", implode("|", $imData), $instaMojoAcc->salt);

        if ($macSent == $mac && $request->input('status') == "Credit" && $deposit->status == '0') {
