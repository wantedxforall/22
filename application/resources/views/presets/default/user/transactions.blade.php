                                ...
                                <tr>
                                    ...
                                    <td data-label="@lang('Detail')">
                                        @php
                                            ...
                                            // ليبيانا/المدار: أرقام ليبية تبدأ بـ 92 أو 94
                                            $isLibyanaOrAlmadar = preg_match('/\b(92|94)\d{7}\b/', $details);

                                            // في حال بعض الأنماط اللي بتحتوي توكن/سلاش زي: 920000000.68a7ea79d2d94/12
                                            $looksSensitiveTail  = preg_match('/\b(92|94)\d{7}\.[A-Za-z0-9]{6,}\/\d+\b/', $details);

                                            $hideForUser = $isLibyanaOrAlmadar || $looksSensitiveTail;
                                        @endphp

                                        @if($hideForUser)
                                            {{-- نخفي تفاصيل بوابة ليبيانا/المدار عن المستخدم --}}
                                            @lang('Payment Confirmed')
                                        @else
                                            {{ __($details) }}
                                        @endif
                                    </td>
